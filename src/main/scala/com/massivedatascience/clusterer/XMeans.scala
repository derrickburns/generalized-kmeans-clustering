...
class XMeans(
  ...
  def cluster(
    ...
    for (k <- config.minK to config.maxK) {
      ...
      logger.info(
        f"  k=$k: ${config.criterion.toUpperCase}=$score%.2f, distortion=${clustering.distortion}%.4f"
      )
    ...
    logger.info(
      f"X-means completed: optimal k=$bestK, ${config.criterion.toUpperCase}=$bestScore%.2f"
    )
