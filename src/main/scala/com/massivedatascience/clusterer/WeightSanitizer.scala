...
object WeightSanitizer {
  ...
  /** Sanitize an RDD of weighted vectors by filtering out zero weights and clamping extreme values
    ...
    * @param removeZeroWeights
    *   if true, filter out vectors with weight below MIN_VALID_WEIGHT
    * @return
    *   sanitized RDD
    */
  def sanitize(
      data: RDD[WeightedVector],
      removeZeroWeights: Boolean = true
  ): RDD[WeightedVector] = {
  ...
  /** Sanitize a single weighted vector by clamping weight to safe range.
    *
    * @param vector
    *   input weighted vector
    * @return
    *   sanitized weighted vector
    */
