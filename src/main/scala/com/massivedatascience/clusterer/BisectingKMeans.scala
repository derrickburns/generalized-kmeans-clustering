 ...
 *
 * @param maxIterationsPerSplit Maximum iterations for each bisection
 * @param minClusterSize Minimum size for a cluster to be split
 * @param splitCriterion Criterion for selecting cluster to split:
 *                       "largest" - split cluster with most points
 ...
 * 2. Repeatedly selects a cluster and splits it into two using k-means (k=2)
 * 3. Continues until reaching target k clusters
 *
 ...
 *
 * Algorithm:
 * 1. Start: all data in one cluster
 * 2. While num_clusters < k:
 *    a. Select largest cluster (or highest cost cluster)
 *    b. Split it into 2 using k-means with k=2
 *    c. Add both sub-clusters to active clusters
 * 3. Return final k clusters
 *
 * Works with any Bregman divergence.
 *
 * @param config Configuration parameters
 * @param baseClusterer Clusterer to use for each split (default: ColumnTrackingKMeans)
 */
class BisectingKMeans(
  ...
  private def bisectingCluster(
    ...
    while (clusters.length < targetK && iteration < targetK * 2) {
      ...
      logger.info(f"Iteration $iteration: Splitting cluster with ${toSplit.size} points, " +
    ...
    logger.info(f"Bisecting k-means completed with k=${clusters.length}, " +
   ...
   */
  private def selectClusterToSplit(clusters: List[ClusterNode]): (ClusterNode, List[ClusterNode]) = {
  ...
  private def buildFinalClustering(
      ...
      pointOps: BregmanPointOps,
      data: RDD[BregmanPoint]): ClusteringWithDistortion = {
