...
/** Algebraic Data Type representing cluster assignment strategies.
  *
  * This ADT defines a composable, testable algebra for expressing how points are assigned to clusters. Instead of
  * implicit string-based dispatch in AssignmentStrategy, this provides:
...
/** Cross-join based assignment for squared Euclidean distance.
  *
  * This strategy performs a cross join between points and centers, computes distances using broadcast centers, and
  * selects the closest center for each point. Optimized for squared Euclidean distance.
  *
...
/** RDD-based assignment using map-side distance computation.
  *
  * This strategy converts the DataFrame to RDD, broadcasts centers, and computes distances in a map operation. This is
  * the traditional Spark MLlib approach.
  *
  * Performance characteristics:
...
/** DataFrame-based assignment using UDF for distance computation.
  *
  * This strategy uses a UDF to compute distances to all centers for each point, avoiding RDD conversion. Useful for
  * integration with DataFrame-based pipelines.
  *
  * Performance characteristics:
...
/** Conditional assignment plan that selects strategy based on data characteristics.
  *
  * This meta-strategy analyzes the data (e.g., number of centers, divergence type) and selects the most appropriate
  * assignment plan. This enables automatic optimization.
  *
  * @param defaultPlan
...
/** Interpreter for executing AssignmentPlan strategies.
  *
  * This class implements the interpreter pattern, translating AssignmentPlan ADT values into concrete DataFrame
  * operations. It separates strategy definition from execution, enabling:
  ...
  */
class AssignmentPlanInterpreter(ops: BregmanPointOps, centers: CenterStore) extends Serializable with Logging {
  ...
  private def executeCrossJoin(plan: CrossJoinAssignmentPlan, df: DataFrame): DataFrame = {
    ...
    val distanceUDF = udf((features: org.apache.spark.ml.linalg.Vector, center: org.apache.spark.ml.linalg.Vector) => {
    ...
    val window = org.apache.spark.sql.expressions.Window.partitionBy("row_id").orderBy(col("distance").asc)
  ...
  private def executeRDDMap(plan: RDDMapAssignmentPlan, df: DataFrame): DataFrame = {
    ...
    val assignmentsRDD = df.rdd.map { row =>
      ...
      val point         = ops.toPoint(com.massivedatascience.linalg.WeightedVector.fromInhomogeneousWeighted(features, 1.0))
    ...
    val newSchema = org.apache.spark.sql.types.StructType(
      schema.fields :+ org.apache.spark.sql.types.StructField(plan.predictionCol, org.apache.spark.sql.types.IntegerType, nullable = false)
    )
