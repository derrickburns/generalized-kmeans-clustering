...
class BregmanCoClusteringModel(val result: BregmanCoClusteringResult, pointOps: BregmanPointOps)
  ...
  /** Predict cluster assignments for new matrix entries.
    *
    * @param entries
    *   RDD of new matrix entries
    * @return
    *   RDD of (entry, (rowCluster, colCluster)) pairs
    */
  ...
  /** Predict cluster assignments for new rows given their column patterns.
    *
    * @param rowEntries
    *   RDD of (rowIndex, Seq[MatrixEntry]) representing new rows
    * @return
    *   RDD of (rowIndex, rowCluster) pairs
    */
  ...
  /** Predict cluster assignments for new columns given their row patterns.
    *
    * @param colEntries
    *   RDD of (colIndex, Seq[MatrixEntry]) representing new columns
    * @return
    *   RDD of (colIndex, colCluster) pairs
    */
  ...
  /** Reconstruct the original matrix using the block structure.
    ...
    * @return
    *   RDD of reconstructed matrix entries
    */
  ...
  /** Get row cluster characteristics.
    ...
    *   Original matrix entries
    * @return
    *   Statistics for each row cluster
    */
  ...
  /** Get column cluster characteristics.
    ...
    *   Original matrix entries
    * @return
    *   Statistics for each column cluster
    */
...
case class CoClusteringModelSummary(
  ...
  def toText: String = {
    ...
    rowStats.zipWithIndex.foreach { case (stats, idx) =>
      sb.append(
        f"  Cluster $idx: ${stats.uniqueIndices} rows, ${stats.totalEntries} entries, mean=${stats.mean}%.3f\n"
      )
    }
    ...
    colStats.zipWithIndex.foreach { case (stats, idx) =>
      sb.append(
        f"  Cluster $idx: ${stats.uniqueIndices} columns, ${stats.totalEntries} entries, mean=${stats.mean}%.3f\n"
      )
    }
