 ...
 * 
 * @param beta Inverse temperature parameter controlling soft assignment sharpness.
 *             Higher values = sharper assignments (approaches hard clustering).
 *             Lower values = softer assignments (more fuzzy membership).
 * @param minMembership Minimum membership probability to avoid numerical issues
 * @param maxIterations Maximum number of iterations
 * @param convergenceThreshold Convergence threshold for membership changes
 * @param computeObjective Whether to compute and track the soft clustering objective
 */
 ...
 * @param objective Final soft clustering objective value
 ...
 * 
 * This implements probabilistic clustering where each point has a membership
 * probability for each cluster, computed using:
 * 
 * p(cluster c | point x) ∝ exp(-β * D_φ(x, μ_c))
 * 
 * where β is the inverse temperature parameter and D_φ is the Bregman divergence.
 * 
 * Higher β values lead to sharper (more decisive) assignments, while lower β
...
case class BregmanSoftKMeans(config: BregmanSoftKMeansConfig = BregmanSoftKMeans.defaultConfig)
   ...
   * 
   * @param maxIterations Maximum iterations (overrides config if different)
   * @param pointOps Bregman point operations
   * @param data Input data points
   * @param initialCenters Initial cluster centers
   * @return Sequence of soft clustering results (one per initial center set)
   */
  def cluster(
      maxIterations: Int,
    ...
    logger.info(s"Starting Bregman soft clustering with β=${config.beta}, ${initialCenters.length} runs")
    
    val actualMaxIterations = if (maxIterations != config.maxIterations) maxIterations else config.maxIterations
   ...
   * @param data Input data points
   * @param initialCenters Initial cluster centers
   * @return Soft clustering result with membership probabilities
   */
  def clusterSoft(
      maxIterations: Int,
      pointOps: BregmanPointOps,
      data: RDD[BregmanPoint],
      initialCenters: IndexedSeq[BregmanCenter]): SoftClusteringResult = {
    
    val numClusters = initialCenters.length
    logger.info(s"Starting soft clustering: k=$numClusters, β=${config.beta}, maxIter=$maxIterations")
    ...
    while (iteration < maxIterations && !converged) {
      ...
      if (config.computeObjective) {
        ...
        val improvement = (previousObjective - objective) / math.max(math.abs(previousObjective), 1e-10)
        
        logger.debug(f"Iteration $iteration: objective = $objective%.6f, improvement = $improvement%.8f")
  ...
  private def computeCenterMovement(
    ...
    oldCenters.zip(newCenters).map { case (oldCenter, newCenter) =>
...
object BregmanSoftKMeans {
   ...
   * 
   * @param beta Higher values → sharper assignments, lower values → softer assignments
   */
