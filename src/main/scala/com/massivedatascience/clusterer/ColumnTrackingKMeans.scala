...
object ColumnTrackingKMeans {
  ...
  /** Find the closest cluster from a given set of clusters
    ...
    *   the point
    * @return
    *   the assignment of that cluster to the point
    */
  def bestAssignment(
  ...
  /** Find the closest cluster assignment to a given point
    ...
    *   point
    * @param assignment
    *   the current assignment of the point
    * @param round
    *   the current round
    * @param centers
    *   the cluster centers
    * @return
    *   the new assignment for the point
    */
  def reassignment(
    pointOps: BregmanPointOps,
    point: BregmanPoint,
    ...
    val nonStationaryCenters = centers.withFilter(c =>
      c.movedSince(assignment.round) &&
        c.center.weight > pointOps.weightThreshold
    )
    val stationaryCenters = centers.withFilter(c =>
      !c.movedSince(assignment.round) &&
        c.center.weight > pointOps.weightThreshold
    )
    ...
    assignment match {
      case a: Assignment if !a.isAssigned =>
        bestAssignment(pointOps, round, point, stationaryCenters, bestNonStationary)
  ...
  def updatedAssignments(
    ...
    points.zip(previousAssignments).mapPartitionsWithIndex[Assignment] { (index, assignedPoints) =>
      val rand = new XORShiftRandom(r ^ (index << 16))
      assignedPoints.map { case (point, assignment) =>
...
case class ColumnTrackingKMeans(config: KMeansConfig = DefaultKMeansConfig)
  ...
  /** Replace empty or weak clusters with new centers derived from points in strong clusters
    ...
    *   current assignments of points to clusters
    * @param centers
    *   current cluster centers
    * @return
    *   updated centers with empty clusters replaced when possible
    */
  private[this] def backFilledCenters(
    ...
    if (weakClusters.nonEmpty && round < config.maxRoundsToBackfill) {
      ...
      val replacements: IndexedSeq[CenterWithHistory] = if (strongClusters.isEmpty) {
        ...
        val pointsInStrongClusters = points
        ...
        try {
          ...
          val y = pointsInStrongClusters.takeSample(
            withReplacement = false,
            math.min(weakClusters.length, pointsInStrongClusters.count().toInt),
            rand.nextLong()
          )

          val result = y.zipWithIndex
            .map { case ((_, pt), i) =>
              if (i < weakClusters.length) {
                ...
                logger.error(
                  s"Index $i out of bounds for weakClusters array of length ${weakClusters.length}"
                )
  ...
  private[this] def latestCenters(
    ...
    if (config.addOnly) {
      val results = completeMovedCentroids(
        points,
        pointOps,
        currentAssignments,
        previousAssignments,
      ...
      val changes = deltasOfMovedCentroids(
        points,
        pointOps,
        currentAssignments,
        previousAssignments,
  ...
  /** Computes all centroids, but only returns centroids of changed clusters. * A previous
    ...
    * @param previousAssignments
    *   previous assignments of points
    * @param numCenters
    *   current number of non-empty clusters
    * @return
    *   complete centroids of clusters that have moved
    */
  private[this] def completeMovedCentroids[T <: WeightedVector](
    ...
    withBroadcast(ops) { (bcPointOps: Broadcast[BregmanPointOps]) =>
      points
        .zipPartitions(assignments, previousAssignments) {
          (x: Iterator[T], y: Iterator[Assignment], z: Iterator[Assignment]) =>
            val pointOps = bcPointOps.value
            val centroids: Array[MutableWeightedVector] =
              Array.tabulate(numCenters)(i => pointOps.make(i))
  ...
  /** @param points
    ...
    *   current number of non-empty clusters
    * @return
    *   deltas to clusters that have moved
    */
  ...
  /** Create a K-Means clustering of the input and report on the resulting distortion
    ...
    * @return
    *   the distortion of the clustering on the points and the cluster centers (model)
    */
  def cluster(
    maxIterations: Int,
    pointOps: BregmanPointOps,
    ...
    ): (RDD[Assignment], IndexedSeq[CenterWithHistory]) = {

      require(assignments.getStorageLevel.useMemory)

      val newAssignments = withBroadcast(centers) { bcCenters =>
        sync[Assignment](
          s"assignments round $round",
