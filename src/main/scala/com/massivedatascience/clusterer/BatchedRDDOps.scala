 ...
 * 
 * This class combines multiple operations that would otherwise require separate
 * RDD transformations into single, more efficient operations.
 */
object BatchedRDDOps {
   ...
   * 1. Point assignment to clusters
   * 2. Distortion calculation  
   * 3. Centroid accumulation for cluster updates
   * 
   * @param points RDD of points to process
   * @param centers Current cluster centers
   * @param pointOps Distance operations
   * @return Tuple of (assignments, total distortion, centroid contributions)
   */
  def assignmentAndCentroidsBatch(
      points: RDD[BregmanPoint],
      centers: IndexedSeq[BregmanCenter],
      pointOps: BregmanPointOps): (RDD[ColumnTrackingKMeans.Assignment], Double, Array[WeightedVector]) = {
    ...
    val batchResults = points.mapPartitions { pointIter =>
      ...
      logger.debug(s"Processed partition with ${assignments.length} points, distortion: ${totalDistortion}")
    ...
    try {
      ...
      logger.info(s"Batch processing completed: distortion=${totalDistortion}, clusters=${aggregatedCentroids.length}")
   ...
   * 
   * This leverages the existing incremental assignment logic that avoids
   * redundant distance calculations for stationary centers.
   * 
   * @param points RDD of points
   * @param currentAssignments Current assignments
   * @param centers Centers with movement history
  ...
  def incrementalAssignmentBatch(
      ...
      round: Int,
      pointOps: BregmanPointOps): RDD[ColumnTrackingKMeans.Assignment] = {
    ...
    logger.info(s"Round ${round}: ${movedCenters.length} moved, ${stationaryCenters.length} stationary centers")
   ...
   * @param previousAssignments Previous assignments (optional)
   * @return Statistics map
   */
  def computeStatsBatch(
      assignments: RDD[ColumnTrackingKMeans.Assignment],
      previousAssignments: Option[RDD[ColumnTrackingKMeans.Assignment]] = None): Map[String, Double] = {
    ...
    }.reduce { case ((dist1, assigned1, unassigned1, clusters1), (dist2, assigned2, unassigned2, clusters2)) =>
    ...
    val avgClusterSize = if (nonEmptyClusters > 0) assignedCount.toDouble / nonEmptyClusters else 0.0
    ...
    val assignmentChanges = previousAssignments.map { prevAssignments =>
      assignments.zip(prevAssignments)
