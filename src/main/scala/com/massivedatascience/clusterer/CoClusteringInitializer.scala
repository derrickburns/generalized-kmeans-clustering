 ...
 * 
 * This class provides various initialization methods that can significantly
 * impact the quality and convergence speed of co-clustering algorithms.
 */
class CoClusteringInitializer(
    numRowClusters: Int,
    numColClusters: Int,
    pointOps: BregmanPointOps,
    seed: Long = System.currentTimeMillis()) extends Serializable {
  ...
  def spectralInit(
    ...
    val rowSums = data.groupBy(_.rowIndex).map { case (rowIdx, entries) =>
      rowIdx -> entries.map(_.value).sum
    }.collect().toMap
    
    val colSums = data.groupBy(_.colIndex).map { case (colIdx, entries) =>
  ...
  def multiStrategyInit(
      ...
      strategies: Seq[String] = Seq("random", "balanced", "kmeans++")): (Map[Long, Int], Map[Long, Int]) = {
  ...
  private def computeRowMarginals(
      data: RDD[MatrixEntry], 
      rowIndices: Array[Long]): Map[Long, BregmanPoint] = {
  ...
  private def computeColMarginals(
      data: RDD[MatrixEntry], 
      colIndices: Array[Long]): Map[Long, BregmanPoint] = {
  ...
  private def computeRowDensities(
      data: RDD[MatrixEntry], 
      rowIndices: Array[Long]): Map[Long, Double] = {
    
    val rowCounts = data.groupBy(_.rowIndex).map { case (rowIdx, entries) =>
      rowIdx -> entries.size
    }.collect().toMap
  ...
  private def computeColDensities(
      data: RDD[MatrixEntry], 
      colIndices: Array[Long]): Map[Long, Double] = {
    
    val colCounts = data.groupBy(_.colIndex).map { case (colIdx, entries) =>
      colIdx -> entries.size
    }.collect().toMap
   ...
   */
  private def clusterByDensity(
      densities: Map[Long, Double], 
      numClusters: Int): Map[Long, Int] = {
  ...
  private def evaluateInitialization(
      ...
      colClusters: Map[Long, Int]): Double = {
...
object CoClusteringInitializer {
  ...
  def apply(
      numRowClusters: Int,
      numColClusters: Int,
      pointOps: BregmanPointOps): CoClusteringInitializer = {
  ...
  def withSeed(
      ...
      numColClusters: Int,
      pointOps: BregmanPointOps,
      seed: Long): CoClusteringInitializer = {
  ...
  def initialize(
      ...
      numColClusters: Int,
      pointOps: BregmanPointOps,
      strategy: String = "random"): (Map[Long, Int], Map[Long, Int]) = {
