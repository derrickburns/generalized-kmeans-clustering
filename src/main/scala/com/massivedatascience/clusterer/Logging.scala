 ...
 *
 * This trait eliminates the need for each class to initialize its own logger
 * and provides helper methods for common logging patterns.
 */
trait Logging extends Serializable {

  /**
   * Lazy logger instance initialized with the class name.
   * Marked @transient to avoid serialization issues.
   */
  ...
  protected def logConvergence(totalIterations: Int, finalDistortion: Double): Unit = {
    logger.info(f"Converged after $totalIterations iterations, final distortion=$finalDistortion%.4f")
  }
  ...
  protected def logClusteringComplete(
      ...
      k: Int,
      iterations: Int,
      distortion: Double): Unit = {
    logger.info(f"$algorithmName completed: k=$k, iterations=$iterations, distortion=$distortion%.4f")
  }
