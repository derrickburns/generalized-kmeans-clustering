...
/** Strategy for sampling data in mini-batch k-means.
  *
  * This trait unifies different batch sampling strategies (full batch, fixed mini-batch, decaying
  * samples) under a single abstraction. It handles:
...
trait MiniBatchScheduler extends Serializable {
  ...
  /** Get learning rate for the given iteration.
    *
    * The learning rate determines how much new batch statistics affect center updates. Common
    * patterns:
...
/** Full batch scheduler - use entire dataset each iteration.
  *
  * This is standard (non-mini-batch) k-means. Each iteration uses all data points with learning
  * rate = 1.0.
  *
  * Characteristics:
...
case class DecayingMiniBatchScheduler(
  ...
  require(
    minFraction > 0.0 && minFraction <= maxFraction,
    s"Invalid fractions: min=$minFraction, max=$maxFraction"
  )
  ...
  override def name: String =
    s"decayingMiniBatch(min=$minFraction, max=$maxFraction, growth=$growthRate)"
