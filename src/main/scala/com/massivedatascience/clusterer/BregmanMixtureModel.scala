 ...
 * @param convergenceThreshold Convergence threshold for log-likelihood
 * @param minMixingWeight Minimum mixing weight to avoid degeneracy
 * @param regularization Regularization parameter for mixing weights
 * @param initializationMethod Method for initializing parameters
 */
case class BregmanMixtureConfig(
  ...
  require(minMixingWeight > 0.0 && minMixingWeight < 1.0, 
 ...
 * 
 * @param center The component center (mean parameter)
 * @param mixingWeight The mixing weight (prior probability) of this component
 * @param componentId Unique identifier for this component
 */
case class MixtureComponent(
    center: BregmanCenter,
    mixingWeight: Double,
    componentId: Int) {
  
  require(mixingWeight >= 0.0 && mixingWeight <= 1.0, 
 ...
 * 
 * @param components The estimated mixture components
 * @param logLikelihood Final log-likelihood value
 * @param responsibilities RDD of posterior probabilities for each point
 * @param iterations Number of EM iterations performed
...
case class BregmanMixtureResult(
  ...
  def bic(numDataPoints: Long, dimensionality: Int): Double = {
    val numParameters = numComponents * (dimensionality + 1) - 1 // centers + mixing weights - constraint
 ...
 * 
 * This estimates mixture models of the form:
 * p(x) = Σ_k π_k * p_k(x)
 * 
 * where π_k are mixing weights and p_k(x) are exponential family distributions
 * corresponding to Bregman divergences.
 * 
 * The likelihood for each component is:
 * p_k(x) ∝ exp(-D_φ(x, μ_k))
 * 
...
case class BregmanMixtureModel(config: BregmanMixtureConfig = BregmanMixtureModel.defaultConfig) {
   ...
   * @param initialCenters Optional initial centers (if not provided, will use initialization method)
  ...
  def fit(
      ...
      initialCenters: Option[IndexedSeq[BregmanCenter]] = None): BregmanMixtureResult = {
    
    require(numComponents > 0, s"Number of components must be positive, got: $numComponents")
    
    logger.info(s"Estimating Bregman mixture model: k=$numComponents, method=${config.initializationMethod}")
    ...
    val (initialComponents, initialResponsibilities) = initializeParameters(
      data, numComponents, pointOps, initialCenters)
    
    logger.info(s"Initialized mixture model with $numComponents components on $numDataPoints data points")
    ...
    while (iteration < config.maxIterations && !converged) {
      ...
      logger.debug(f"Iteration $iteration: log-likelihood = $logLikelihood%.6f, improvement = $improvement%.8f")
  ...
  private def initializeParameters(
      ...
      numComponents: Int,
      pointOps: BregmanPointOps,
      initialCenters: Option[IndexedSeq[BregmanCenter]]): (IndexedSeq[MixtureComponent], RDD[(BregmanPoint, Array[Double])]) = {
    ...
    val centers = initialCenters.getOrElse {
      config.initializationMethod match {
        ...
        case "random" =>
          ...
          data.takeSample(false, numComponents, 42L).zipWithIndex.map { case (point, _) =>
            pointOps.toCenter(point)
          }.toIndexedSeq
          
        case _ =>
          throw new IllegalArgumentException(s"Unknown initialization method: ${config.initializationMethod}")
      }
    }
  ...
  private def updateParameters(
      responsibilities: RDD[(BregmanPoint, Array[Double])],
      numComponents: Int,
      pointOps: BregmanPointOps): IndexedSeq[MixtureComponent] = {
  ...
  private def computeLogLikelihood(
      responsibilities: RDD[(BregmanPoint, Array[Double])],
      components: IndexedSeq[MixtureComponent],
      pointOps: BregmanPointOps): Double = {
...
object BregmanMixtureModel {
  ...
  def quick(
      data: RDD[BregmanPoint],
      numComponents: Int,
      pointOps: BregmanPointOps): BregmanMixtureResult = {
