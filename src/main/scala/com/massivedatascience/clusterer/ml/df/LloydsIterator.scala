...
/** Configuration for Lloyd's algorithm.
  ...
  * @param tol
  *   convergence tolerance (max center movement)
  * @param kernel
...
trait LloydsIterator extends Logging {
  ...
  /** Run Lloyd's algorithm until convergence or max iterations.
    *
    * @param df
    *   input DataFrame with features column
    * @param featuresCol
    *   name of features column
    * @param weightCol
    *   optional name of weight column
...
class DefaultLloydsIterator extends LloydsIterator {
  ...
  override def run(
    ...
    logInfo(
      s"Starting Lloyd's algorithm with k=${config.k}, maxIter=${config.maxIter}, " +
    ...
    try {
      while (iter < config.maxIter && !converged) {
        ...
        val assigned =
          config.assigner.assign(currentDF, featuresCol, weightCol, centers, config.kernel)
        ...
        val newCenters =
          config.updater.update(assignedCached, featuresCol, weightCol, config.k, config.kernel)
        ...
        if (emptyCount > 0) {
          ...
          logWarning(
            s"Iteration $iter: handled $emptyCount empty clusters " +
