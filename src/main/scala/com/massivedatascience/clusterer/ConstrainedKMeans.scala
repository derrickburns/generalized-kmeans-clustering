 ...
 *
 * @param mustLink Set of (point_id, point_id) pairs that must be in same cluster
 * @param cannotLink Set of (point_id, point_id) pairs that cannot be in same cluster
 */
case class Constraints(
   ...
   */
  def getForbiddenClusters(
      pointId: Long,
      assignments: Map[Long, Int]): Set[Int] = {
 ...
 *
 * @param constraints Pairwise constraints (must-link and cannot-link)
 * @param violationPenalty Penalty for constraint violations (Infinity = hard constraints)
 * @param maxViolations Maximum constraint violations allowed (0 = hard constraints)
 */
 ...
 *
 * Algorithm:
 * 1. Standard k-means with modified assignment step
 * 2. When assigning point to cluster, respect constraints:
 ...
 *
 * @param config Configuration with constraints and penalties
 * @param baseClusterer Underlying clusterer (default: ColumnTrackingKMeans)
 */
class ConstrainedKMeans(
  ...
  def cluster(
    ...
    val indexedData = data.zipWithUniqueId().map { case (point, id) =>
      (id, point)
    }.cache()
  ...
  private def constrainedCluster(
    ...
    logger.info(f"Constrained k-means completed: $iteration iterations, cost=$cost%.4f, violations=$violations")
  ...
  private def assignWithConstraints(
      ...
      pointOps: BregmanPointOps): RDD[(Long, Int)] = {
  ...
  private def hasConverged(
      ...
      pointOps: BregmanPointOps): Boolean = {
  ...
  private def computeCost(
    ...
    indexedData.join(assignments).map { case (id, (point, cluster)) =>
...
object ConstrainedKMeans {
  ...
  def withSoftConstraints(constraints: Constraints, penalty: Double = 1000.0): ConstrainedKMeans = {
    new ConstrainedKMeans(ConstrainedKMeansConfig(
      constraints = constraints,
