...
/** Configuration for Bregman core-set construction.
  *
  * @param coresetSize
  *   Target size of the core-set
  * @param epsilon
  *   Approximation parameter (smaller = better quality, larger core-set)
  * @param sensitivity
  *   Sensitivity computation strategy
  * @param seed
  *   Random seed for reproducibility
  * @param minSamplingProb
  *   Minimum sampling probability to avoid extreme weights
  * @param maxWeight
...
/** Result of core-set construction with quality metrics.
  *
  * @param coreset
  *   The constructed core-set
  * @param originalSize
  *   Size of the original dataset
  * @param compressionRatio
  *   Ratio of core-set size to original size
  * @param totalSensitivity
  *   Sum of all sensitivity scores
  * @param avgSamplingProb
  *   Average sampling probability used
  * @param config
...
class BregmanCoreset(config: CoresetConfig) extends Serializable {
  ...
  /** Construct a core-set from the given points.
    *
    * @param points
    *   RDD of points to create core-set from
    * @param k
    *   Number of clusters (affects sensitivity computation)
    * @param pointOps
    *   Bregman point operations
    * @return
    *   CoresetResult containing the core-set and metadata
    */
  def buildCoreset(points: RDD[BregmanPoint], k: Int, pointOps: BregmanPointOps): CoresetResult = {
    ...
    val samplingProbs = sensitivities.map { case (point, sensitivity) =>
      val prob = math.min(
        1.0,
        math.max(config.minSamplingProb, config.coresetSize * sensitivity / totalSensitivity)
      )
...
object BregmanCoreset {
  ...
  /** Quick core-set construction with reasonable defaults.
    *
    * @param points
    *   Points to create core-set from
    * @param k
    *   Number of clusters
    * @param pointOps
    *   Bregman operations
    * @param compressionRatio
    *   Target compression ratio (0.01 = 1% of original size)
    * @return
    *   CoresetResult
    */
  def quick(
