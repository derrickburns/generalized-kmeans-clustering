...
class BregmanSoftKMeansTestSuite extends AnyFunSuite with LocalClusterSparkContext {
  ...
  test("BregmanSoftKMeans should produce different results with different beta values") {
    ...
    val ambiguousPointSoft = softMemberships
      .find(_._1.homogeneous.toArray.sameElements(Array(1.3, 1.3, 1.0)))
      .map(_._2)
      .getOrElse(Array(0.5, 0.5))

    val ambiguousPointSharp = sharpMemberships
      .find(_._1.homogeneous.toArray.sameElements(Array(1.3, 1.3, 1.0)))
      .map(_._2)
      .getOrElse(Array(0.5, 0.5))
    ...
    assert(
      avgSoftEntropy >= avgSharpEntropy,
  ...
  test("BregmanSoftKMeans should compute reasonable effective number of clusters") {
    val points = sc.parallelize((1 to 50).map { i =>
      val x =
        if (i <= 25) 1.0 + 0.1 * scala.util.Random.nextGaussian()
    ...
    assert(
      effectiveNumClusters >= 1.0 && effectiveNumClusters <= 3.0,
