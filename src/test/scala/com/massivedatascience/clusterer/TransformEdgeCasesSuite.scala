...
class TransformEdgeCasesSuite extends AnyFunSuite {
  ...
  test("HaarWavelet with power-of-2 lengths") {
    ...
    for (length <- lengths) {
      val vector = WeightedVector(Vectors.dense(Array.fill(length)(scala.util.Random.nextGaussian())), 1.0)
  ...
  test("RandomIndexEmbedding with different dimensions") {
    ...
    for (embedding <- embeddings) {
      ...
      assert(result.inhomogeneous.toArray.zip(result2.inhomogeneous.toArray).forall {
        case (a, b) => math.abs(a - b) < 1e-10
      })
    }
  }
  ...
  test("embedding preserves weight under all transformations") {
    ...
    for (embedding <- embeddings) {
      for (weight <- weights) {
        ...
        try {
          val result = embedding.embed(vector)
          assert(result.weight == weight, s"Weight not preserved by ${embedding.getClass.getSimpleName}")
        } catch {
          case _: Exception =>
