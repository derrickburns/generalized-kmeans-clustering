...
class KMeansSuite extends AnyFunSuite with LocalClusterSparkContext {
  ...
  test("coverage") {
    ...
    val data =
      sc.parallelize[Vector](Array.fill(1000)(Vectors.dense(Array.fill(20)(r.nextDouble()))))
    ...
    KMeans.train(
      ...
      maxIterations = 10,
      runs = 1,
      clustererName = MultiKMeansClusterer.MINI_BATCH_10
    )
  }
  ...
  test("more clusters than points") {
    val data =
      sc.parallelize[Vector](Array(Vectors.dense(1.0, 2.0, 3.0), Vectors.dense(1.0, 3.0, 4.0)), 2)
  ...
  test("k-means|| initialization") {
    ...
    case class VectorWithCompare(x: Vector) extends Ordered[VectorWithCompare] {
      @Override def compare(that: VectorWithCompare): Int = {
        if (
          this.x.toArray.foldLeft[Double](0.0)((acc, x) => acc + x * x) >
    ...
    assert(
      model.clusterCenters
        .sortBy(VectorWithCompare)
        .zip(points.sortBy(VectorWithCompare))
  ...
  test("empty clusters handling") {
    ...
    assert(
      firstThreeClusters != lastThreeClusters,
      "The two groups of points should be in different clusters"
    )
  }
  ...
  test("KMeans model serialization") {
    ...
    assert(
      originalPredictions.sameElements(deserializedPredictions),
